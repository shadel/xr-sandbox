...
const canvasSandboxVirtualization = new CanvasSandboxVirtualization(
  containerElement,
  sandboxUrl,
  canvasId
);
...
customFunctionButton.addEventListener("click", () => {
  const functionBody = `
    ...
    streamCanvasData();
  `;
  canvasSandboxVirtualization.injectFunction(
    "customYellowCircle",
    functionBody
  );
...
hookFunctionButton.addEventListener("click", () => {
  const hookBody = `
